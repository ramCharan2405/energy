...
import { buyEnergyOnBlockchain } from "@/utils/ethereum";
...
export default function BuyEnergy() {
  ...
  const buyEnergyMutation = useMutation({
    mutationFn: async (data: {
      ...
      try {
        ...
        const totalCostEth = (
          parseFloat(data.amount) * parseFloat(data.ratePerKWh)
        ).toFixed(6);
        ...
        const blockchainTxHash = await buyEnergyOnBlockchain(
          data.blockchainListingId,
          data.amount,
          totalCostEth
        );

        // Then update the backend database with the real transaction hash
        return apiRequest("POST", "/api/transactions/buy", {
          ...data,
          blockchainTxHash,
        });
      } catch (error) {
        ...
        // Fallback: Create transaction without blockchain integration
        toast({
          title: "Blockchain Error",
          description: "Processing transaction with mock data...",
          variant: "destructive",
        });
    ...
    onSuccess: async () => {
      ...
      await refreshUser(); // This will update user balances
      ...
      toast({
        ...
        description:
          "Real blockchain transaction completed! Check your wallet.",
      });
      setPurchaseAmounts({});
    },
    onError: (error: any) => {
  ...
  const handlePurchase = async (listing: EnergyListing) => {
    ...
    await buyEnergyMutation.mutateAsync({
      ...
      blockchainListingId: listing.blockchainListingId || 1, // Fallback for existing listings
    });
  };
  ...
  if (sortBy === "price") {
    sortedListings = [...sortedListings].sort(
      (a, b) => parseFloat(a.ratePerKWh) - parseFloat(b.ratePerKWh)
    );
  } else if (sortBy === "amount") {
    sortedListings = [...sortedListings].sort(
      (a, b) => parseFloat(b.amountKWh) - parseFloat(a.amountKWh)
    );
  }
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2 text-sm text-muted-foreground">
            ...
            <span data-testid="text-active-listings-count">
      ...
      {listingsLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {sortedListings.map((listing: any) => (
            <Card
              key={listing.id}
              className="bg-card shadow border border-border"
            ...
            >
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <div
                      className={`w-10 h-10 bg-gradient-to-br ${generateAvatar(
                    ...
                    <div>
                      <p
                        ...
                        data-testid={`text-seller-address-${listing.id}`}
                      >
                ...
                <div className="space-y-3 mb-4">
                  <div className="flex justify-between">
                    ...
                    <span
                      className="font-medium text-foreground"
                      data-testid={`text-available-amount-${listing.id}`}
                    >
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className="font-medium text-foreground"
                      data-testid={`text-rate-${listing.id}`}
                    >
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      ...
                      data-testid={`text-total-value-${listing.id}`}
                    >
