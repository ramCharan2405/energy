...
export const users = pgTable("users", {
  ...
  energyBalance: decimal("energy_balance", { precision: 18, scale: 8 }).notNull().default("0"),
...
export const transactions = pgTable("transactions", {
  ...
  sellerId: varchar("seller_id").notNull().references(() => users.id),
  listingId: varchar("listing_id").notNull().references(() => energyListings.id),
  amountKWh: decimal("amount_kwh", { precision: 18, scale: 8 }).notNull(),
  ratePerKWh: decimal("rate_per_kwh", { precision: 18, scale: 18 }).notNull(),
...
export const insertEnergyListingSchema = createInsertSchema(energyListings).omit({
  id: true,
  createdAt: true,
});
